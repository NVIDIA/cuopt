...
You need **both** `libcuopt` and the Python package (`update_q` / `sequence_solve` are in Python, not only `libcuopt.so`).
...
```bash
python -c "from cuopt.linear_programming import DataModel, SolverSettings, Solve; print('update_q', hasattr(DataModel(), 'update_q')); print('sequence_solve', hasattr(SolverSettings(), 'sequence_solve'))"
```

Expect `True` / `True`. If `update_q` is missing, Python is using a released wheel, not this build.

## 5. Run cache reuse

Same `DataModel` instance. Barrier + `sequence_solve`. `update_q` changes **only** the linear objective (`c`). Leave `A`, `Q`, and bounds unchanged. QP only (quadratic objective, no quadratic constraints / SOC). First solve must be Optimal.
...
dm.update_q(new_c)               # only c; same dm
