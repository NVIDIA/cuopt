...
- cache:  t=0 store, t=1..K-1 ``update_q`` + reuse (skip convert/presolve/scaling)
...
def summarize(path_name: str, config: str, baseline: list[dict], session_rows: list[dict]) -> dict:
    ...
    assert cache_reuse_warm == len(warm_s), (
        f"{path_name}/{config}: expected update_q cache reuse on every warm solve, "
...
def main():
    ...
    print(
        ...
        "IR=0 bound_free_vars=0. Session warm solves use update_q (cache reuse).\n"
    )
