                    ...
                    bool deterministic)
{
  ...
  cuopt::math_optimization::mip_solver_settings_t<int, double> settings;
  ...
  bool parsing_failed = false;
  {
    ...
    try {
      mps_data_model =
        cuopt::math_optimization::io::read_mps<int, double>(file_path, input_mps_strict);
  ...
  settings.presolver                     = cuopt::math_optimization::presolver_t::Default;
  settings.reliability_branching         = reliability_branching;
  settings.clique_cuts                   = -1;
  settings.seed                          = 42;
  cuopt::math_optimization::benchmark_info_t benchmark_info;
  settings.benchmark_info_ptr = &benchmark_info;
  auto start_run_solver       = std::chrono::high_resolution_clock::now();
  auto solution = cuopt::math_optimization::solve_mip(&handle_, mps_data_model, settings);
  CUOPT_LOG_INFO(
  ...
  int sol_found  = int(solution.get_termination_status() ==
                        cuopt::math_optimization::mip_termination_status_t::FeasibleFound ||
                      solution.get_termination_status() ==
                        cuopt::math_optimization::mip_termination_status_t::Optimal);
  ...
  // and infeasibility-flagged instances emit "opt=Infeasible".
  {
    ...
    switch (solution.get_termination_status()) {
      case cuopt::math_optimization::mip_termination_status_t::Optimal:
        _status_str = "Optimal";
        break;
      case cuopt::math_optimization::mip_termination_status_t::FeasibleFound:
        _status_str = "FeasibleFound";
        break;
      case cuopt::math_optimization::mip_termination_status_t::TimeLimit:
        _status_str = "TimeLimit";
        break;
      case cuopt::math_optimization::mip_termination_status_t::Infeasible:
  ...
  int is_optimal =
    solution.get_termination_status() == cuopt::math_optimization::mip_termination_status_t::Optimal
      ? 1
      : 0;
