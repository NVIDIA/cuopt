...
├── cuopt_remote_data.proto         # Data definitions (problems, solutions, settings) — codegen-generated
...
└── server/
    ...
    ├── grpc_pipe_io.cpp            # Low-level pipe read/write helpers
    ├── grpc_incumbent_proto.hpp    # Incumbent proto build/parse (codegen target)
    ├── grpc_server_logger.{hpp,cpp}# Server-side logging utilities
...
| File | Purpose |
|------|---------|
| `cpp/src/grpc/cuopt_remote.proto` | Base message definitions (job status, enums) |
| `cpp/src/grpc/cuopt_remote_data.proto` | Data messages (problems, settings, solutions, field IDs) — generated by `cpp/codegen/generate_conversions.py` |
...
struct grpc_client_config_t {
  ...
  int timeout_seconds        = 0;     // Max wait for job completion (0 = no limit)
  bool stream_logs           = false; // Stream solver logs from server
  std::function<void(const std::string&)> log_callback;

  // Incumbent callback for MIP solves
  ...
  // Debug / test options
  std::function<void(const std::string&)> debug_log_callback;
  bool enable_debug_log    = false;  // Controlled by CUOPT_GRPC_DEBUG env var
