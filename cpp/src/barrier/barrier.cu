                                                             ...
                                                             f_t& max_residual)
{
  ...
  max_residual = 0.0;
  {
    ...
    cub::DeviceTransform::Transform(
      ...
      stream_view_.value());
    RAFT_CHECK_CUDA(stream_view_);
    // diag/inv_diag host copies removed: form_augmented(false) uses d_diag_ directly;
    // form_adat uses d_inv_diag; initial_point only needs them during setup (already set there).
  }
  ...
  // Save primal_rhs (d_h_ before SpMV) for augmented RHS; affine=primal_residual, CC=0.
  rmm::device_uvector<f_t> d_primal_rhs_save(lp.num_rows, stream_view_);

  {
    ...
    // Capture primal_rhs before SpMV overwrites d_h_.
    raft::copy(d_primal_rhs_save.data(), data.d_h_.data(), lp.num_rows, stream_view_);
  ...
  if (use_augmented) {
    ...
    raft::copy(d_augmented_rhs.data() + lp.num_cols, d_primal_rhs_save.data(), lp.num_rows, stream_view_);
...
void barrier_solver_t<i_t, f_t>::compute_affine_rhs(iteration_data_t<i_t, f_t>& data)
{
  ...
  raft::copy(data.d_h_.data(), data.d_primal_residual_.data(), data.d_primal_residual_.size(), stream_view_);
  raft::copy(data.d_dual_rhs_.data(), data.d_dual_residual_.data(), data.d_dual_residual_.size(), stream_view_);
  data.d_bound_rhs_.resize(data.d_bound_residual_.size(), stream_view_);
  raft::copy(data.d_bound_rhs_.data(), data.d_bound_residual_.data(), data.d_bound_residual_.size(), stream_view_);
  data.d_dw_.resize(data.d_bound_residual_.size(), stream_view_);
  raft::copy(data.d_dw_.data(), data.d_bound_residual_.data(), data.d_bound_residual_.size(), stream_view_);
...
void barrier_solver_t<i_t, f_t>::compute_cc_rhs(iteration_data_t<i_t, f_t>& data, f_t& new_mu)
{
  ...
  thrust::fill(rmm::exec_policy(stream_view_), data.d_dual_rhs_.begin(), data.d_dual_rhs_.end(), f_t(0));
  thrust::fill(rmm::exec_policy(stream_view_), data.d_bound_rhs_.begin(), data.d_bound_rhs_.end(), f_t(0));
...
lp_status_t barrier_solver_t<i_t, f_t>::solve(f_t start_time, lp_solution_t<i_t, f_t>& solution)
{
  ...
  try {
    ...
    data.d_upper_bounds_.resize(data.upper_bounds.size(), stream_view_);
    raft::copy(data.d_upper_bounds_.data(), data.upper_bounds.data(), data.upper_bounds.size(), stream_view_);
    ...
    raft::copy(data.d_restrict_u_.data(), data.restrict_u_.data(), data.restrict_u_.size(), stream_view_);
    f_t dual_objective =
    ...
    while (iter < iteration_limit) {
      ...
      i_t status = gpu_compute_search_direction(data, max_affine_residual);
      if (settings.concurrent_halt != nullptr && *settings.concurrent_halt == 1) {
        settings.log.printf("Barrier solver halted\n");
        return lp_status_t::CONCURRENT_LIMIT;
      }
      ...
      status = gpu_compute_search_direction(data, max_corrector_residual);
      if (settings.concurrent_halt != nullptr && *settings.concurrent_halt == 1) {
        settings.log.printf("Barrier solver halted\n");
        return lp_status_t::CONCURRENT_LIMIT;
      }
