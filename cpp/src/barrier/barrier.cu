 ...
 public:
  iteration_data_t(const lp_problem_t<i_t, f_t>& lp,
    ...
    : upper_bounds(num_upper_bounds),
      ...
      d_cone_hinv2_dx_(0, lp.handle_ptr->get_stream()),
  ...
  void form_augmented(bool first_call = false)
  {
    ...
    if (has_soc) {
      ...
      raft::copy(cone_offsets_host.data(), cones().cone_offsets.data(), n_cones + 1, stream_view_);
    ...
    if (first_call) {
      ...
      if (has_soc) {
        scatter_hinv2_into_augmented(cones(),
                                     device_augmented.x,
                                     d_cone_csr_indices_,
                                     d_cone_Q_values_,
                                     handle_ptr->get_stream());
                          ...
                          rmm::device_uvector<f_t>& y)
  {
    ...
    if (has_soc) {
      const i_t m_c = cone_entry_count();
      accumulate_cone_hinv2_matvec(raft::device_span<const f_t>(d_x1.data() + cone_start(), m_c),
                                   cones(),
  ...
  rmm::device_uvector<f_t> d_cone_hinv2_dx_;
...
int barrier_solver_t<i_t, f_t>::initial_point(iteration_data_t<i_t, f_t>& data)
{
  ...
  if (settings.barrier_dual_initial_point == -1 || settings.barrier_dual_initial_point == 0) {
    ...
    data.z.ensure_positive_skip_range(epsilon_adjust, data.cone_start(), data.cone_entry_count());
  } else {
...
#endif
  // Make sure (w, x, v, z) > 0; skip cone vars — handled by shift_into_interior below
                                                            ...
                                                            f_t& complementarity_residual_norm)
{
  ...
  if (has_soc) {
    ...
    auto cone_dot                 = data.cones().scratch.template get_slot<0>();
...
{
  const bool has_soc         = data.has_cones();
  const bool skip_cone_range = has_soc && static_cast<i_t>(x.size()) >= data.cone_end();

  auto ratio_test = [] HD(const thrust::tuple<f_t, f_t> t) {
  ...
  if (skip_cone_range) {
    ...
    if (cs > 0) {
      alpha = std::min(alpha,
                       data.transform_reduce_helper_.transform_reduce(
    ...
    i_t tail_start = cs + mc;
    i_t tail_size  = static_cast<i_t>(x.size()) - tail_start;
    if (tail_size > 0) {
      alpha = std::min(alpha,
                       data.transform_reduce_helper_.transform_reduce(
                         thrust::make_zip_iterator(dx.data() + tail_start, x.data() + tail_start),
                         thrust::minimum<f_t>(),
                         ratio_test,
                         f_t(1),
                         tail_size,
                                                             ...
                                                             f_t& max_residual)
{
  ...
  max_residual = 0.0;
  {
    ...
    // diag = z ./ x  (cone entries are overwritten by H^{-2} in the augmented matrix)
...

  {
    ...
    cub::DeviceTransform::Transform(
      ...
      stream_view_.value());
    RAFT_CHECK_CUDA(stream_view_);
    // tmp3 <- tmp3 .+ dual_rhs - complementarity_target
    // tmp4 <- inv_diag .* tmp3
    cub::DeviceTransform::Transform(
...

  {
    ...
    if (has_soc) {
      ...
      if (cone_var_start > 0) {
        recover_linear_dz(
          raft::device_span<const f_t>(data.d_complementarity_target_.data(), cone_var_start),
          raft::device_span<const f_t>(data.d_z_.data(), cone_var_start),
          raft::device_span<const f_t>(data.d_dx_.data(), cone_var_start),
          raft::device_span<const f_t>(data.d_x_.data(), cone_var_start),
          raft::device_span<f_t>(data.d_dz_.data(), cone_var_start));
      }
    } else {
      recover_linear_dz(cuopt::make_span(data.d_complementarity_target_),
                        cuopt::make_span(data.d_z_),
                        cuopt::make_span(data.d_dx_),
                        cuopt::make_span(data.d_x_),
                        cuopt::make_span(data.d_dz_));
    }
    RAFT_CHECK_CUDA(stream_view_);
  ...
  iteration_data_t<i_t, f_t>& data, f_t mu, f_t& mu_aff, f_t& sigma, f_t& new_mu)
{
  ...
  if (has_soc) {
    ...
    auto [cone_p, cone_d] =
      compute_cone_step_length(data.cones(),
                               ...
                               std::min(step_primal_aff, step_dual_aff),
                               stream_view_);
    f_t cone_aff    = std::min(cone_p, cone_d);
    step_primal_aff = std::min(step_primal_aff, cone_aff);
    step_dual_aff   = step_primal_aff;
  }
  ...
  f_t mu_denom            = data.complementarity_degree(data.x.size(), data.n_upper_bounds);
...
void barrier_solver_t<i_t, f_t>::compute_cc_rhs(iteration_data_t<i_t, f_t>& data, f_t& new_mu)
{
  ...
  fill_linear_cc_rhs(raft::device_span<f_t>(data.d_complementarity_xz_rhs_.data(), linear_size),
                                              ...
                                              lp_solution_t<i_t, f_t>& solution)
{
  try {
    ...
    f_t complementarity_residual_norm =
      std::max(vector_norm_inf<i_t, f_t>(linear_xz_span, stream_view_),
               vector_norm_inf<i_t, f_t>(data.complementarity_wv_residual, stream_view_));
    if (has_soc) {
      f_t cone_complementarity_norm = f_t(0);
      i_t off                       = data.cone_start();
      for (auto q_k : lp.second_order_cone_dims) {
        f_t cone_dot = f_t(0);
        for (i_t j = 0; j < q_k; ++j) {
          cone_dot += data.complementarity_xz_residual[off + j];
        }
