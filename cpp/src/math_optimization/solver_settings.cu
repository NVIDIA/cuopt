...
solver_settings_t<i_t, f_t>::solver_settings_t() : pdlp_settings(), mip_settings()
{
  ...
  float_parameters = {
    ...
    {CUOPT_MIP_HYPER_HEURISTIC_PROBING_WORK_TIME_SCALE, &mip_settings.heuristic_params.probing_work_time_scale, f_t(0.0), std::numeric_limits<f_t>::infinity(), f_t(3.0e9), "numerator of the probing work ceiling; divided by nnz + n_cand * avg_col_len (0 disables)"},
