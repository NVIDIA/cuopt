...
solver_settings_t<i_t, f_t>::solver_settings_t() : pdlp_settings(), mip_settings()
{
  ...
  float_parameters = {
    ...
    {CUOPT_MIP_SEMICONTINUOUS_BIG_M, &mip_settings.sc_big_m, f_t(1.0), std::numeric_limits<f_t>::infinity(), f_t(1e10), "big-M value for semi-continuous variables with no finite upper bound"},
   };
