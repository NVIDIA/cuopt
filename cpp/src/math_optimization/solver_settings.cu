...
solver_settings_t<i_t, f_t>::solver_settings_t() : pdlp_settings(), mip_settings()
{
  ...
  int_parameters = {
    ...
    {CUOPT_BARRIER_PRESOLVE_REMOVE_FIXED_VARIABLES, &pdlp_settings.barrier_presolve_remove_fixed_variables, -1, 1, -1, "Remove variables with equal lower and upper bounds during barrier presolve: -1 automatic (enabled), 0 disabled, 1 enabled"},
