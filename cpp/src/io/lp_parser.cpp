                                 ...
                                 const lp_parser_t<i_t, f_t>& parser)
{
  ...
  for (const auto& block : parser.quadratic_constraint_blocks) {
    ...
    std::vector<i_t> q_offsets;
    build_symmetric_q_csr(
      block.quad_triples, n_vars, q_values, q_col_indices, q_offsets);
    std::vector<i_t> q_row_indices;
    q_row_indices.reserve(q_values.size());
    for (i_t r = 0; r < n_vars; ++r) {
      for (i_t p = q_offsets[r]; p < q_offsets[r + 1]; ++p) {
        q_row_indices.push_back(r);
      }
    }
