...
{
  cuopt_expects(num_entries > 0,
                error_type_t::ValidationError,
                "quadratic constraint must have at least one matrix entry");
  cuopt_expects(row_index != nullptr && col_index != nullptr && coeff != nullptr,
                error_type_t::ValidationError,
                "row_index, col_index, and coeff cannot be null");
  ...
  qc.rows.assign(row_index, row_index + num_entries);
  qc.cols.assign(col_index, col_index + num_entries);
  qc.vals.assign(coeff, coeff + num_entries);
