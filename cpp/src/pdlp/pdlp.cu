...
pdlp_solver_t<i_t, f_t>::pdlp_solver_t(problem_t<i_t, f_t>& op_problem,
  ...
  : climber_strategies_(batch_size_handler(op_problem, settings)),
    ...
    unscaled_primal_avg_solution_{is_cupdlpx_ && !settings.get_pdlp_warm_start_data().is_populated() ? 0 : problem_size,
                                  stream_view_},
    unscaled_dual_avg_solution_{is_cupdlpx_ && !settings.get_pdlp_warm_start_data().is_populated() ? 0 : problem_size,
                                stream_view_},
    ...
    average_termination_strategy_{handle_ptr_,
                                  ...
                                  is_cupdlpx_ ? 0 : primal_size_h_,
                                  is_cupdlpx_ ? 0 : dual_size_h_,
