 ...
 */
cuopt::math_optimization::lp_solution_interface_t<int, double>* call_solve_lp(
  cuopt::math_optimization::optimization_problem_interface_t<int, double>* problem_interface,
  cuopt::math_optimization::pdlp_solver_settings_t<int, double>& solver_settings,
  bool is_batch_mode)
{
  raft::common::nvtx::range fun_scope("Call Solve LP");
  cuopt_expects(
    problem_interface->get_problem_category() == cuopt::math_optimization::problem_category_t::LP,
  ...
  auto solution_interface = cuopt::math_optimization::solve_lp(
 ...
 */
cuopt::math_optimization::mip_solution_interface_t<int, double>* call_solve_mip(
  cuopt::math_optimization::optimization_problem_interface_t<int, double>* problem_interface,
  cuopt::math_optimization::mip_solver_settings_t<int, double>& solver_settings)
{
  raft::common::nvtx::range fun_scope("Call Solve MIP");
  cuopt_expects((problem_interface->get_problem_category() ==
                 cuopt::math_optimization::problem_category_t::MIP) or
                  (problem_interface->get_problem_category() ==
                   cuopt::math_optimization::problem_category_t::IP),
                error_type_t::ValidationError,
                "MIP solve cannot be called on an LP problem!");

  // Solve returns unique_ptr<mip_solution_interface_t>
  auto solution_interface = cuopt::math_optimization::solve_mip(problem_interface, solver_settings);
...
std::unique_ptr<solver_ret_t> call_solve(
  cuopt::math_optimization::io::data_model_view_t<int, double>* data_model,
  cuopt::math_optimization::solver_settings_t<int, double>* solver_settings,
  unsigned int flags,
  bool is_batch_mode)
{
  ...
  auto memory_backend = cuopt::math_optimization::get_memory_backend_type();

  solver_ret_t response;

  // Create problem instance and CUDA resources based on memory backend
  if (memory_backend == cuopt::math_optimization::memory_backend_t::GPU) {
    ...
    auto problem = cuopt::math_optimization::optimization_problem_t<int, double>(&handle_);
    cuopt::math_optimization::populate_from_data_model_view(
      &problem, data_model, solver_settings, &handle_);

    // Call appropriate solve function and convert to ret struct
    if (problem.get_problem_category() == math_optimization::problem_category_t::LP) {
      // Solve and get solution interface pointer
      auto lp_solution_ptr =
        std::unique_ptr<math_optimization::lp_solution_interface_t<int, double>>(
          call_solve_lp(&problem, solver_settings->get_pdlp_settings(), is_batch_mode));

      response.lp_ret       = lp_solution_ptr->to_python_lp_ret();
      response.problem_type = math_optimization::problem_category_t::LP;
      ...
      auto mip_solution_ptr =
        std::unique_ptr<math_optimization::mip_solution_interface_t<int, double>>(
          call_solve_mip(&problem, solver_settings->get_mip_settings()));

      response.mip_ret      = mip_solution_ptr->to_python_mip_ret();
      response.problem_type = math_optimization::problem_category_t::MIP;
    ...
    auto cpu_problem = cuopt::math_optimization::cpu_optimization_problem_t<int, double>();
    cuopt::math_optimization::populate_from_data_model_view(
      &cpu_problem, data_model, solver_settings, nullptr);

    // Call appropriate solve function and convert to ret struct
    if (cpu_problem.get_problem_category() == math_optimization::problem_category_t::LP) {
      auto lp_solution_ptr =
        std::unique_ptr<math_optimization::lp_solution_interface_t<int, double>>(
...
static int compute_max_thread(
  const std::vector<cuopt::math_optimization::io::data_model_view_t<int, double>*>& data_models)
{
...
std::pair<std::vector<std::unique_ptr<solver_ret_t>>, double> solve_batch_remote(
  std::vector<cuopt::math_optimization::io::data_model_view_t<int, double>*> data_models,
  cuopt::math_optimization::solver_settings_t<int, double>* solver_settings)
{
  cuopt_expects(
    false,
  ...
  cuopt::math_optimization::solver_settings_t<int, double>* solver_settings)
{
  ...
  if (cuopt::math_optimization::is_remote_execution_enabled()) {
