...
mab_t::mab_t(int n_arms, int seed, std::string bandit_name)
  : mab_arm_stats_(n_arms), mab_rng_(seed), bandit_name(bandit_name)
{
}
                           ...
                           double time_in_miliseconds)
{
  ...
  if (option_id >= 0 && option_id < static_cast<int>(mab_arm_stats_.size())) {
    ...
    if (is_better_than_best_feasible) {
      ...
      reward = factor * (max(0.1, 4.0 - (time_in_miliseconds / 2000)));
    }
