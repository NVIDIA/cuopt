  ...
  mip_solution_t<i_t, f_t>& incumbent)
{
  ...
  f_t lower_bound     = root_objective;
  ...
  while (gap > settings.absolute_mip_gap_tol &&
    ...
    node_stack.pop_back();  // Remove node from the stack
    ...
    // Solve the LP relaxation of the current node using Dual Simplex and the previous basis
    simplex_solver_settings_t lp_settings = settings;
    lp_settings.set_log(false);
    ...
    if (lp_status == dual::status_t::DUAL_UNBOUNDED) {
      ...
      if (leaf_fractional == 0) {
        ...
        if (down_dist < up_dist) {
          node_stack.push_back(up_child.get());
          node_stack.push_back(down_child.get());
        } else {
          node_stack.push_back(up_child.get());
          node_stack.push_back(down_child.get());
        }
