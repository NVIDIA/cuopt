...
mip_status_t branch_and_bound_t<i_t, f_t>::solve_dfs(mip_solution_t<i_t, f_t>& solution)
{
  ...
  // Calculate the LP relaxation of the root node
  ...
  // Check if the MIP has already been solved at the root node
  if (num_fractional == 0) {
    global_variables::mutex_upper.lock();
  ...
  // Apply strong branching as the initial pseudocost for the fractional variables
  ...
  while (gap > settings.absolute_mip_gap_tol &&
    ...
    if (lp_status == dual::status_t::DUAL_UNBOUNDED) {
      ...
      if (leaf_fractional == 0) {
        ...
        const f_t down_val  = std::floor(root_relax_soln.x[branch_var]);
        const f_t up_val    = std::ceil(root_relax_soln.x[branch_var]);
  ...
  if (node_stack.size() == 0 && get_upper_bound<f_t>() == inf) {
...
template <typename i_t, typename f_t>
mip_status_t branch_and_bound_t<i_t, f_t>::solve_bfs(mip_solution_t<i_t, f_t>& solution)
{
  mip_status_t status = mip_status_t::UNSET;
  mip_solution_t<i_t, f_t> incumbent(original_lp.num_cols);
  if (guess.size() != 0) {
  ...
  i_t branch_var =
    pc.variable_selection(fractional, root_relax_soln.x, original_lp.lower, original_lp.upper, log);
  f_t branch_var_val = root_relax_soln.x[branch_var];
