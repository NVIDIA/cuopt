...
inline char feasible_solution_symbol(worker_type_t type, bool log_diving_type)
{
  if (type == HEURISTICS) return 'H';
  if (type == BEST_FIRST) return 'B';
  if (type == SUBMIP) return 'S';
...
template <typename i_t, typename f_t>
void branch_and_bound_t<i_t, f_t>::report_heuristic(f_t obj, worker_type_t type)
{
  if (is_running_) {
    ...
    std::string log_line =
      std::format("{} {:>12} {:>12} {:^+19.6e} {:^+15.6e} {:>8} {:>7} {:^11} {:^11}",
                  feasible_solution_symbol(type, false),
                  "",  // nodes explored
                  "",  // nodes unexplored
                  user_obj,
                  user_lower,
                                                                ...
                                                                worker_type_t heuristic_type)
{
  ...
  settings_.log.debug_format("{} found solution with obj={:.4g}",
                             feasible_solution_symbol(heuristic_type, false),
                             compute_user_objective(original_lp_, obj));
  ...
  if (!incumbent_.has_incumbent || obj < incumbent_.objective) {
    ...
    if (is_feasible && (!incumbent_.has_incumbent || obj < incumbent_.objective)) {
      ...
      if (current_upper_bound > upper_bound_.load()) {
        report_heuristic(obj, heuristic_type);
        success = true;
      }
    ...
    for (const std::vector<f_t>& uncrushed_solution : to_repair) {
      ...
      if (is_feasible) {
        ...
        if (!incumbent_.has_incumbent || repaired_obj < incumbent_.objective) {
          ...
          report_heuristic(repaired_obj, HEURISTICS);
  ...
  branch_and_bound_worker_t<i_t, f_t>* worker)
{
  ...
  switch (worker->worker_type) {
    ...
    case SUBMIP:  // This is used for solving the DFS of the sub-MIP.
      ...
      // These should never be called for selecting which variable to branch
    case HEURISTICS:
    case NUM_WORKER_TYPES:
...
struct nondeterministic_policy_t : tree_update_policy_t<i_t, f_t> {
  ...
  {
    bnb.add_feasible_solution(obj, x, node->depth, worker->worker_type);
  }
  ...
  void update_objective_estimate(mip_node_t<i_t, f_t>* node,
                                 ...
                                 const std::vector<f_t>& x) override
  {
    if (worker->worker_type == worker_type_t::BEST_FIRST) {
...
struct deterministic_diving_policy_t
                                                ...
                                                const std::vector<f_t>& x) override
  {
    ...
    switch (this->worker.diving_type) {
      ...
      case worker_type_t::COEFFICIENT_DIVING: {
  ...
  logger_t& log)
{
  ...
  if (worker->recompute_bounds && worker->orbital_fixing && worker->worker_type == BEST_FIRST) {
...
void branch_and_bound_t<i_t, f_t>::dive_with(diving_worker_t<i_t, f_t>* worker, i_t backtrack_limit)
{
  ...
  if (worker->worker_type != SUBMIP) { diving_worker_pool_.return_worker_to_pool(worker); }
}
...
bool branch_and_bound_t<i_t, f_t>::launch_diving_worker(bfs_worker_t<i_t, f_t>* bfs_worker)
{
  ...
  diving_worker->worker_type = strategy;
                                                ...
                                                std::string_view log_prefix)
{
  ...
  if (settings_.submip_settings.enable_cpufj) {
    ...
    f_t work_limit = 1.0;
    submip_fj_cpu_worker.create_climber_from_simplex_lp(submip_bnb.original_lp_,
...
template <typename i_t, typename f_t>
void get_integer_list(const std::vector<f_t>& lower,
...
template <typename i_t, typename f_t>
void apply_rins_fixings(const std::vector<f_t>& node_solution,
                        const std::vector<i_t>& fractional,
                        const std::vector<f_t>& current_incumbent,
                        const simplex_solver_settings_t<i_t, f_t>& settings,
                        i_t max_var_fixed,
...
template <typename i_t, typename f_t>
void extend_variable_fixings(const std::vector<f_t>& obj_coeffs,
                                        ...
                                        const std::vector<f_t>& node_solution)
{
  ...
  if (rins_worker->orbital_fixing) { rins_worker->orbital_fixing->disable(); }
  logger_t log;
  log.log = false;
  ...
  std::vector<f_t>& current_sol     = rins_worker->leaf_solution.x;
  std::vector<i_t> fractional;
  ...
  std::vector<i_t> integer_list;
  get_integer_list(lower, upper, var_types_, settings_.fixed_tol, integer_list);
  ...
  while (solver_status_ == mip_status_t::UNSET && is_running_) {
    ...
    apply_rins_fixings(node_solution,
                       fractional,
                       current_incumbent,
                       settings_,
                       max_var_fixed,
    ...
    if (prev_num_fixed == num_var_fixed) {
      // RINS neighbourhood 2: Search into the entire list of integer variables where the current
      ...
      if (prev_num_fixed == num_var_fixed) {
        std::vector<std::tuple<f_t, i_t, f_t>> candidates;
        extend_variable_fixings(rins_worker->leaf_problem.objective,
                                lower,
                                upper,
                                fractional,
                                current_sol,
    ...
    if (fixrate < settings_.submip_settings.min_fixrate_cap ||
      ...
      if (is_feasible) {
        ...
        if (settings_.submip_settings.enable_cpufj) {
          ...
          f_t work_limit = 1.0;
          submip_fj_cpu_worker.create_climber_from_simplex_lp(
            rins_worker->leaf_problem,
            var_types_,
  ...
  rins_worker_pool_.return_worker_to_pool(rins_worker);
}
...
mip_status_t branch_and_bound_t<i_t, f_t>::solve(mip_solution_t<i_t, f_t>& solution)
{
  ...
  if (guess_.size() != 0) {
    ...
    if (feasible) {
      ...
      settings_.log.print_format("Setting initial MIP start. Obj={:+.6e}",
...
#pragma omp taskwait depend(in : *clique_signal)
    ...
    if (enable_root_cut_cpufj && !settings_.deterministic && settings_.num_threads >= 2 &&
      ...
      root_fj_cpu_worker.create_climber_from_simplex_lp(
  ...
  if (enable_root_cut_cpufj && cut_info.has_cuts()) {
    ...
    root_fj_cpu_worker.create_climber_from_simplex_lp(original_lp_,
                                                      var_types_,
                                                      root_relax_soln_.x,
                                                      settings_,
                                                      "[RootCut CPUFJ] ",
...
void branch_and_bound_t<i_t, f_t>::run_deterministic_coordinator(const csr_matrix_t<i_t, f_t>& Arow)
{
  ...
  if (num_diving_workers > 0) {
    ...
    std::vector<worker_type_t> diving_types;
  ...
  if (deterministic_diving_workers_ && deterministic_diving_workers_->size() > 0) {
    ...
    for (const auto& worker : *deterministic_diving_workers_) {
      ...
      switch (worker.diving_type) {
        case worker_type_t::PSEUDOCOST_DIVING: type_str = "PC"; break;
        case worker_type_t::LINE_SEARCH_DIVING: type_str = "LS"; break;
        case worker_type_t::GUIDED_DIVING: type_str = "GD"; break;
        case worker_type_t::COEFFICIENT_DIVING: type_str = "CD"; break;
        default: break;
      }
  ...
  PoolT& pool, WorkerTypeGetter get_worker_type)
{
  ...
  for (const auto* sol : all_solutions) {
    if (sol->objective < current_upper) {
      ...
      worker_type_t worker_type = get_worker_type(pool, sol->worker_id);
  ...
  const bb_event_batch_t<i_t, f_t>& events)
{
  ...
  while (event_idx < events.events.size() || heuristic_idx < heuristic_solutions.size()) {
    ...
    if (process_heuristic) {
      ...
      if (new_upper < std::numeric_limits<f_t>::infinity()) {
        report_heuristic(new_upper, HEURISTICS);
  ...
  deterministic_process_worker_solutions(*deterministic_workers_,
                                         [](const deterministic_bfs_worker_pool_t<i_t, f_t>&, int) {
                                           return worker_type_t::BEST_FIRST;
                                         });
