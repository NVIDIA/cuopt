...
template <typename i_t, typename f_t>
void fill_knapsack_constraints(const simplex::user_problem_t<i_t, f_t>& problem,
                               std::vector<knapsack_constraint_t<i_t, f_t>>& knapsack_constraints,
                               simplex::csr_matrix_t<i_t, f_t>& A)
{
                        ...
                        cuopt::timer_t& timer)
{
  ...
  std::vector<knapsack_constraint_t<i_t, f_t>> knapsack_constraints;
  simplex::csr_matrix_t<i_t, f_t> A(problem.num_rows, problem.num_cols, 0);
...
#define INSTANTIATE(F_TYPE)                                                                    \
  template void find_initial_cliques<int, F_TYPE>(                                             \
    simplex::user_problem_t<int, F_TYPE> & problem,                                            \
  ...
  template void build_clique_table<int, F_TYPE>(                                               \
    const simplex::user_problem_t<int, F_TYPE>& problem,                                       \
