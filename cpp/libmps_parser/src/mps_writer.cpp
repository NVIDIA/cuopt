...
void mps_writer_t<i_t, f_t>::write(const std::string& mps_file_path)
{
  ...
  i_t n_variables   = problem_.get_variable_names().size();
  i_t n_constraints = problem_.get_constraint_upper_bounds().size();

  std::vector<f_t> objective_coefficients(problem_.get_objective_coefficients().size());
  std::vector<f_t> constraint_lower_bounds(problem_.get_constraint_lower_bounds().size());
  std::vector<f_t> constraint_upper_bounds(problem_.get_constraint_upper_bounds().size());
  ...
  // data_model_view_t may contain device or host pointers(?). Handle both w/ a raft copy
  cudaMemcpy(objective_coefficients.data(),
             problem_.get_objective_coefficients().data(),
             problem_.get_objective_coefficients().size() * sizeof(f_t),
             cudaMemcpyDefault);
  cudaMemcpy(constraint_lower_bounds.data(),
             problem_.get_constraint_lower_bounds().data(),
             problem_.get_constraint_lower_bounds().size() * sizeof(f_t),
             cudaMemcpyDefault);
  cudaMemcpy(constraint_upper_bounds.data(),
             problem_.get_constraint_upper_bounds().data(),
             problem_.get_constraint_upper_bounds().size() * sizeof(f_t),
             cudaMemcpyDefault);
  cudaMemcpy(variable_lower_bounds.data(),
             problem_.get_variable_lower_bounds().data(),
             problem_.get_variable_lower_bounds().size() * sizeof(f_t),
             cudaMemcpyDefault);
  cudaMemcpy(variable_upper_bounds.data(),
             problem_.get_variable_upper_bounds().data(),
             problem_.get_variable_upper_bounds().size() * sizeof(f_t),
             cudaMemcpyDefault);
  cudaMemcpy(variable_types.data(),
             problem_.get_variable_types().data(),
             problem_.get_variable_types().size() * sizeof(char),
             cudaMemcpyDefault);
  cudaMemcpy(row_types.data(),
             problem_.get_row_types().data(),
             problem_.get_row_types().size() * sizeof(char),
             cudaMemcpyDefault);
  cudaMemcpy(constraint_matrix_offsets.data(),
             problem_.get_constraint_matrix_offsets().data(),
             problem_.get_constraint_matrix_offsets().size() * sizeof(i_t),
             cudaMemcpyDefault);
  cudaMemcpy(constraint_matrix_indices.data(),
             problem_.get_constraint_matrix_indices().data(),
             problem_.get_constraint_matrix_indices().size() * sizeof(i_t),
             cudaMemcpyDefault);
  cudaMemcpy(constraint_matrix_values.data(),
             problem_.get_constraint_matrix_values().data(),
             problem_.get_constraint_matrix_values().size() * sizeof(f_t),
             cudaMemcpyDefault);
  ...
  for (size_t i = 0; i < (size_t)n_constraints; i++) {
    ...
    f_t rhs;
    if (std::isinf(constraint_lower_bounds[i])) {
