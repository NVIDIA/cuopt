...
// <int, double> (guarded by MIP_INSTANTIATE_FLOAT / MIP_INSTANTIATE_DOUBLE). The tests above
// only exercise <int, double>; repeat the linkage-relevant ones for <int, float> so a missing
// float instantiation is caught the same way a missing double one would be.
TEST(SolverSettingsWrapperTest, InitialPdlpPrimalAndDualSolutionFloat)
{
  const raft::handle_t handle_{};
  auto stream = handle_.get_stream();

  cuopt::mathematical_optimization::solver_settings_t<int, float> settings{};

  std::vector<float> primal = {1.0f, 2.0f, 3.0f};
  std::vector<float> dual   = {4.0f, 5.0f};

  rmm::device_uvector<float> d_primal = cuopt::device_copy(primal, stream);
  rmm::device_uvector<float> d_dual   = cuopt::device_copy(dual, stream);
...
TEST(SolverSettingsWrapperTest, AddInitialMipSolutionFloat)
{
  const raft::handle_t handle_{};
  auto stream = handle_.get_stream();
  ...
  std::vector<float> initial_solution   = {1.0f, 0.0f, 1.0f};
  rmm::device_uvector<float> d_solution = cuopt::device_copy(initial_solution, stream);
...
TEST(SolverSettingsWrapperTest, SetPdlpWarmStartDataRawPointersFloat)
{
  cuopt::mathematical_optimization::solver_settings_t<int, float> settings{};

  std::vector<float> current_primal_solution             = {0.1f, 0.2f, 0.3f};
  std::vector<float> current_dual_solution               = {0.4f, 0.5f};
  std::vector<float> initial_primal_average              = {0.6f, 0.7f, 0.8f};
  std::vector<float> initial_dual_average                = {0.9f, 1.0f};
  std::vector<float> current_ATY                         = {1.1f, 1.2f, 1.3f};
  std::vector<float> sum_primal_solutions                = {1.4f, 1.5f, 1.6f};
  std::vector<float> sum_dual_solutions                  = {1.7f, 1.8f};
  std::vector<float> last_restart_duality_gap_primal_sol = {1.9f, 2.0f, 2.1f};
  std::vector<float> last_restart_duality_gap_dual_sol   = {2.2f, 2.3f};
  ...
  settings.set_pdlp_warm_start_data(
    ...
    /*primal_size=*/static_cast<int>(current_primal_solution.size()),
    /*dual_size=*/static_cast<int>(current_dual_solution.size()),
    /*initial_primal_weight=*/1.5f,
    /*initial_step_size=*/0.01f,
    /*total_pdlp_iterations=*/10,
    /*total_pdhg_iterations=*/20,
    /*last_candidate_kkt_score=*/1e-3f,
    /*last_restart_kkt_score=*/1e-4f,
    /*sum_solution_weight=*/5.0f,
    /*iterations_since_last_restart=*/7);

  const auto& view = settings.get_pdlp_warm_start_data_view();

  auto as_vector = [](auto span) { return std::vector<float>(span.begin(), span.end()); };
  EXPECT_EQ(as_vector(view.current_primal_solution_), current_primal_solution);
  EXPECT_EQ(as_vector(view.current_dual_solution_), current_dual_solution);
  EXPECT_EQ(as_vector(view.last_restart_duality_gap_dual_solution_),
            last_restart_duality_gap_dual_sol);
  EXPECT_FLOAT_EQ(view.initial_primal_weight_, 1.5f);
  EXPECT_EQ(view.total_pdlp_iterations_, 10);
  EXPECT_EQ(view.iterations_since_last_restart_, 7);
}
