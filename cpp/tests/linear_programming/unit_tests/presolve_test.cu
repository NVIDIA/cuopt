...
TEST(pslp_presolve, postsolve_accuracy_afiro)
{
  ...
  auto mps_data_model = cuopt::linear_programming::parsers::parse_mps<int, double>(path, true);
...
TEST(pslp_presolve, postsolve_multiple_problems)
{
  ...
  for (const auto& [name, expected_obj] : instances) {
    ...
    auto mps_data_model =
      cuopt::linear_programming::parsers::parse_mps<int, double>(path, name == "afiro_original");
