...
TEST(pslp_presolve, postsolve_accuracy_afiro)
{
  ...
  auto mps_data_model = cuopt::math_optimization::io::read_mps<int, double>(path, true);
...
TEST(pslp_presolve, compare_with_no_presolve)
{
  ...
  settings_no_presolve.method    = cuopt::math_optimization::method_t::PDLP;
  ...
  settings_pslp.method                               = cuopt::math_optimization::method_t::PDLP;
...
TEST(pslp_presolve, postsolve_multiple_problems)
{
  ...
  for (const auto& [name, expected_obj] : instances) {
    ...
    auto mps_data_model =
      cuopt::math_optimization::io::read_mps<int, double>(path, name == "afiro_original");
