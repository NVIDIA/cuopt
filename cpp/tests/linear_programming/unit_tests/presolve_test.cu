...
class simplex_problem : public ::testing::TestWithParam<std::string> {};
...
TEST_P(simplex_problem, round_trip)
{
  init_logger_t log("", true);
  raft::handle_t handle{};

  auto path       = mathematical_optimization::test::make_path_absolute(GetParam());
  auto mps        = cuopt::mathematical_optimization::io::read_mps<int, double>(path, false);
  auto op_problem = mps_data_model_to_optimization_problem(&handle, mps);
  auto user_problem = cuopt_optimization_problem_to_user_problem<int, double>(&handle, op_problem);

  simplex::simplex_solver_settings_t<int, double> settings;

  // Forward: range-form user_problem -> simplex standard form (A x = b, l <= x <= u),
  // appending one slack/artificial column per row.
  simplex::lp_problem_t<int, double> simplex_problem(&handle, 0, 0, 0);
  std::vector<int> new_slacks;
  simplex::dualize_info_t<int, double> dualize_info;
  convert_user_problem(user_problem, settings, simplex_problem, new_slacks, dualize_info);
  ASSERT_FALSE(dualize_info.solving_dual)
    << "the default (non-barrier) path must not dualize the problem";

  // var_types must span the full simplex problem; the appended slack/artificial
  // columns are continuous.
  std::vector<simplex::variable_type_t> full_var_types = user_problem.var_types;
  full_var_types.resize(simplex_problem.num_cols, simplex::variable_type_t::CONTINUOUS);
  ...
  // and the inverse drops exactly those.
  EXPECT_EQ(recovered.num_rows, user_problem.num_rows);
  EXPECT_EQ(recovered.num_cols, user_problem.num_cols);
  EXPECT_EQ(recovered.objective, user_problem.objective);
  EXPECT_EQ(recovered.lower, user_problem.lower);
  EXPECT_EQ(recovered.upper, user_problem.upper);
  EXPECT_EQ(recovered.var_types, user_problem.var_types);

  // row_sense / rhs: '>=' rows are folded to '<=' by negating coefficients and
  // rhs in the forward pass, so they come back as 'L' with a negated rhs. '<=',
  // '==', and range rows ('==' with a finite range value here) are recovered
  // unchanged.
  std::vector<char> expected_row_sense = user_problem.row_sense;
  std::vector<double> expected_rhs     = user_problem.rhs;
  for (int i = 0; i < user_problem.num_rows; ++i) {
    if (user_problem.row_sense[i] == 'G') {
      expected_row_sense[i] = 'L';
      expected_rhs[i]       = -user_problem.rhs[i];
    }
  }
  EXPECT_EQ(recovered.row_sense, expected_row_sense);
  for (int i = 0; i < user_problem.num_rows; ++i) {
    EXPECT_NEAR(recovered.rhs[i], expected_rhs[i], 1e-9) << "rhs[" << i << "]";
  }

  // Range rows survive as the same row indices, and each range width is |r|.
  EXPECT_EQ(recovered.num_range_rows, user_problem.num_range_rows);
  EXPECT_EQ(recovered.range_rows, user_problem.range_rows);
  ASSERT_EQ(recovered.range_value.size(), user_problem.range_value.size());
  for (std::size_t k = 0; k < recovered.range_value.size(); ++k) {
    EXPECT_NEAR(recovered.range_value[k], std::abs(user_problem.range_value[k]), 1e-9)
      << "range_value[" << k << "]";
  }

  // (2) Round-trip invariant: re-running the forward conversion on the recovered
  // problem reproduces the original standard-form problem exactly (same slack
  // columns, same matrix, same bounds).
  simplex::lp_problem_t<int, double> simplex_problem2(&handle, 0, 0, 0);
  std::vector<int> new_slacks2;
  simplex::dualize_info_t<int, double> dualize_info2;
  convert_user_problem(recovered, settings, simplex_problem2, new_slacks2, dualize_info2);

  EXPECT_EQ(new_slacks2, new_slacks);
  ASSERT_EQ(simplex_problem2.num_rows, simplex_problem.num_rows);
  ASSERT_EQ(simplex_problem2.num_cols, simplex_problem.num_cols);
  for (int j = 0; j < simplex_problem.num_cols; ++j) {
    EXPECT_DOUBLE_EQ(simplex_problem2.objective[j], simplex_problem.objective[j])
      << "objective[" << j << "]";
    EXPECT_DOUBLE_EQ(simplex_problem2.lower[j], simplex_problem.lower[j]) << "lower[" << j << "]";
    EXPECT_DOUBLE_EQ(simplex_problem2.upper[j], simplex_problem.upper[j]) << "upper[" << j << "]";
  }
  for (int i = 0; i < simplex_problem.num_rows; ++i) {
    EXPECT_DOUBLE_EQ(simplex_problem2.rhs[i], simplex_problem.rhs[i]) << "rhs[" << i << "]";
  }
  EXPECT_EQ(simplex_problem2.A.m, simplex_problem.A.m);
  EXPECT_EQ(simplex_problem2.A.n, simplex_problem.A.n);
  ASSERT_EQ(simplex_problem2.A.col_start, simplex_problem.A.col_start);
  const int nnz = simplex_problem.A.col_start[simplex_problem.A.n];
  for (int p = 0; p < nnz; ++p) {
    EXPECT_EQ(simplex_problem2.A.i[p], simplex_problem.A.i[p]) << "A.i[" << p << "]";
    EXPECT_DOUBLE_EQ(simplex_problem2.A.x[p], simplex_problem.A.x[p]) << "A.x[" << p << "]";
  }
}
