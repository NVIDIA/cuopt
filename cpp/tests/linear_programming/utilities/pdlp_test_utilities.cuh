...
static cuopt::math_optimization::optimization_problem_solution_t<i_t, f_t> solve_lp_batch(
  raft::handle_t const* handle_ptr,
  const cuopt::math_optimization::io::mps_data_model_t<i_t, f_t>& mps_data_model,
  const cuopt::math_optimization::pdlp_solver_settings_t<i_t, f_t>& settings)
{
  auto gpu_op = cuopt::math_optimization::mps_data_model_to_optimization_problem<i_t, f_t>(
    handle_ptr, mps_data_model);
  auto batch_settings                                = settings;
  batch_settings.generate_batch_primal_dual_solution = true;
  return cuopt::math_optimization::run_batch_pdlp(gpu_op, batch_settings);
}
...
template <typename i_t, typename f_t>
static cuopt::math_optimization::optimization_problem_solution_t<i_t, f_t> solve_lp_batch_fixed(
  raft::handle_t const* handle_ptr,
  const cuopt::math_optimization::io::mps_data_model_t<i_t, f_t>& mps_data_model,
  cuopt::math_optimization::pdlp_solver_settings_t<i_t, f_t> settings,
  i_t batch_size,
  ...
  bool use_direct_api                                         = false)
{
  ...
  if (use_direct_api) { return cuopt::math_optimization::solve_lp(gpu_op, settings, false); }
  return cuopt::math_optimization::run_batch_pdlp(gpu_op, settings);
}

// Compute on the CPU x * c to check that the returned objective value is correct
static void test_objective_sanity(
  const cuopt::math_optimization::io::mps_data_model_t<int, double>& op_problem,
