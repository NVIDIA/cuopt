...
inline cuopt::init_logger_t dummy_logger(
  const cuopt::math_optimization::solver_settings_t<int, double>& settings)
{
...
int run_single_file(const std::string& file_path,
                    ...
                    cuopt::math_optimization::solver_settings_t<int, double>& settings)
{
  ...
  auto timer          = cuopt::timer_t(settings.get_parameter<double>(CUOPT_TIME_LIMIT));
  {
    ...
    try {
      mps_data_model = cuopt::math_optimization::io::read<int, double>(file_path);
  ...
  std::unique_ptr<cuopt::math_optimization::optimization_problem_interface_t<int, double>>
    problem_interface;

  if (memory_backend == cuopt::math_optimization::memory_backend_t::GPU) {
    handle_ptr = std::make_unique<raft::handle_t>();
    problem_interface =
      std::make_unique<cuopt::math_optimization::optimization_problem_t<int, double>>(
        handle_ptr.get());
  } else {
    problem_interface =
      std::make_unique<cuopt::math_optimization::cpu_optimization_problem_t<int, double>>();
  }

  cuopt::math_optimization::populate_from_mps_data_model(problem_interface.get(), mps_data_model);
  ...
  try {
    if (is_mip) {
      auto& mip_settings = settings.get_mip_settings();
      auto solution = cuopt::math_optimization::solve_mip(problem_interface.get(), mip_settings);
    } else {
      auto& lp_settings = settings.get_pdlp_settings();
      auto solution     = cuopt::math_optimization::solve_lp(problem_interface.get(), lp_settings);
    }
  } catch (const std::exception& e) {
...
int main(int argc, char* argv[])
{
...

  {
    ...
    cuopt::math_optimization::solver_settings_t<int, double> dummy_settings;
