...
cdef create_solution(unique_ptr[solver_ret_t] sol_ret_ptr,
    ...
    if sol_ret.problem_type == ProblemCategory.MIP or sol_ret.problem_type == ProblemCategory.IP: # noqa
        ...
        return Solution(
            ProblemCategory(sol_ret.problem_type).name,
            dict(zip(data_model_obj.get_variable_names(), solution)),
            solve_time,
            primal_solution=solution,
            termination_status=MILPTerminationStatus(termination_status).name,
        ...
        if not is_batch:
            ...
            return Solution(
                ...
                initial_primal_average=initial_primal_average,
                initial_dual_average=initial_dual_average,
                current_ATY=current_ATY,
                sum_primal_solutions=sum_primal_solutions,
                sum_dual_solutions=sum_dual_solutions,
                ...
                total_pdlp_iterations=total_pdlp_iterations,
                total_pdhg_iterations=total_pdhg_iterations,
                last_candidate_kkt_score=last_candidate_kkt_score,
                last_restart_kkt_score=last_restart_kkt_score,
                sum_solution_weight=sum_solution_weight,
                iterations_since_last_restart=iterations_since_last_restart,
                termination_status=(
                    LPTerminationStatus(termination_status).name
                ),
                error_status=ErrorStatus(error_status),
                error_message=str(error_message),
        ...
        return Solution(
            problem_category=ProblemCategory(sol_ret.problem_type).name,
            vars=dict(zip(data_model_obj.get_variable_names(), primal_solution)), # noqa
            solve_time=solve_time,
            primal_solution=primal_solution,
            dual_solution=dual_solution,
            reduced_cost=reduced_cost,
            termination_status=LPTerminationStatus(termination_status).name,
            error_status=ErrorStatus(error_status),
            error_message=str(error_message),
