...
def test_solomon():
    ...
    s.set_time_limit(nodes / 5)
    s.dump_config_file("cfg_file.yaml")
    ...
    # ref_cost = final_cost
    # ref_veh_cost = vehicle_size
    assert cu_status == 0

    d, s = routing.utils.create_data_model_from_yaml("cfg_file.yaml")
    routing_solution = routing.Solve(d, s)
    ...
    assert cu_status == 0
    # FIXME: Deterministic PR
    # assert vehicle_size == ref_veh_cost
    # assert math.fabs((final_cost - ref_cost) / ref_cost) < 0.01
