...
| `set_break_locations()` | Yes | test_vehicle_properties (breaks tests) |
| `add_break_dimension()` | Yes | test_vehicle_properties |
...
| `get_cost_matrix()` | Yes | test_data_model |
| `get_transit_time_matrix()` | Yes | test_solver (PDP getter check) |
...
| `get_break_locations()` | Yes | test_vehicle_properties (test_uniform_breaks, test_non_uniform_breaks) |
| `get_break_dimensions()` | Yes | test_vehicle_properties (same tests) |
...
| `get_min_vehicles()` | Yes | test_solver_settings |
...
| `dump_best_results()` | Yes | test_solver_settings |
| `dump_config_file()` | Yes | save_restore_test |
| `get_time_limit()` | Yes | test_solver_settings (test_min_vehicles, test_max_distance, test_solver_settings_getters) |
| `get_best_results_file_path()` | Yes | test_solver_settings |
| `get_config_file_name()` | Yes | test_solver_settings (test_solver_settings_getters) |
| `get_best_results_interval()` | Yes | test_solver_settings |
...
- **DataModel getters not covered:** `get_transit_time_matrices()`.
- **SolverSettings not covered:** `set_verbose_mode()`.

All **setters** on DataModel and SolverSettings that are present in `vehicle_routing.py` are covered by at least one test. Getters and a few Assignment/SolverSettings methods remain untested.
