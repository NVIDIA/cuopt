...
def test_export_exports_host_and_device_to_host():
    ...
    # matrices are row-major float32; both the host and the device input export
    # to host with the correct values.
    np.testing.assert_array_equal(
        p["cost_matrices"][0]["values"], COST.ravel(order="C")
    )
    np.testing.assert_array_equal(
        p["cost_matrices"][1]["values"], (COST + 1).ravel(order="C")
    )
    assert p["cost_matrices"][1]["vehicle_type"] == 1
    np.testing.assert_array_equal(
        p["order_prizes"], np.array([0, 2, 2, 2, 2], np.float32)
    )
    assert p["capacity_dimensions"][0]["name"] == "demand"
...
def test_export_breaks():
    ...
    np.testing.assert_array_equal(
        p["uniform_breaks"][0]["duration"], np.array([5, 5], np.int32)
    )
    veh0 = p["vehicle_breaks"][0]
    assert veh0["vehicle_id"] == 0 and len(veh0["breaks"]) == 2
    assert veh0["breaks"][1]["earliest"] == 30
    np.testing.assert_array_equal(
        veh0["breaks"][1]["locations"], np.array([3], np.int32)
    )
    assert not _has_device_ref(p)
