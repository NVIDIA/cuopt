...
def extract_shell_commands(notebook_path: str) -> List[str]:
    ...
    try:
        ...
        shell_commands = []
        allowed_commands = [
            'wget', 'curl', 'git', 'python', 'jupyter', 'ls', 'pwd', 'cd', 
            'mkdir', 'rm', 'cp', 'mv', 'chmod', 'unzip', 'tar', 'apt', 'yum', 
            'brew', 'conda', 'npm', 'yarn', 'docker', 'kubectl', 'helm', 
            'aws', 'gcloud', 'az'
        ]
...
def execute_pip_command(cmd: str, verbose: bool = False) -> bool:
    ...
    if verbose:
        ...
        print(f"DEBUG: Command contains 'numpy': {'YES' if 'numpy' in exec_cmd else 'NO'}")
...
def main():
    parser = argparse.ArgumentParser(description='Extract and optionally execute commands from Jupyter notebooks')
    parser.add_argument('notebook_path', help='Path to the Jupyter notebook')
    parser.add_argument('--extract-only', action='store_true', help='Only extract commands, do not execute')
