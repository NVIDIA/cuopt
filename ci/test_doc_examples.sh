...
find_cuopt_libraries() {
    ...
    for search_dir in "${search_dirs[@]}"; do
        ...
        if [ -z "${include_path}" ]; then
            ...
            local found_header
            found_header=$(find "${search_dir}" -name "cuopt_c.h" -path "*/math_optimization/*" 2>/dev/null | head -1)
            ...
            if [ -n "${found_header}" ]; then
                ...
                if echo "${found_header}" | grep -q "/libcuopt/include/"; then
                    # Python package structure: /path/to/libcuopt/include/cuopt/math_optimization/cuopt_c.h
                    ...
                    # Standard installation: /path/to/include/cuopt/math_optimization/cuopt_c.h
