...
- **`InputValidationError, InputRuntimeError, OutOfMemoryError`** — Parser errors from the underlying C++ readers (via ``catch_io_exception``).
- **`RuntimeError`** — If the file extension is not one of the supported suffixes (raised by the C++ ``read`` dispatch).
...
- **`InputValidationError, InputRuntimeError, OutOfMemoryError`** — Parser errors from the underlying C++ reader (via ``catch_io_exception``).
...
- **`ValueError`** — If *model* is not a :class:`DataModel` instance.
