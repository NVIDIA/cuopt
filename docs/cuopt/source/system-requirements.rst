...
.. dropdown:: Minimum Requirements
   ...
   * GNU OpenMP runtime (for multi-threaded cuDSS/barrier solves):
      - pip installs bundle their own; no host ``libgomp`` needed
      - conda and source builds need ``libgomp.so.1`` on the host -- install via ``libgomp1`` (Debian/Ubuntu) or ``libgomp`` (RHEL/Rocky/Fedora)
