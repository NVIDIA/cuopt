...
- **File extensions**: `.hpp`/`.cpp` for C++ host code; `.cuh`/`.cu` for CUDA;
  `.h` reserved for the C ABI surface (`cpp/include/cuopt/math_optimization/cuopt_c.h`).
...
The C API surface is intentionally narrow — `cpp/include/cuopt/math_optimization/cuopt_c.h`.
...
**API surface**
- Any change to `cpp/include/cuopt/math_optimization/cuopt_c.h` — flag as ABI-sensitive
